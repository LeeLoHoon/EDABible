...
관리자로 로그인한 뒤 앱 제목을 2초 안에 5번 탭하면 관리자 모드가 토글됩니다. 관리자가 정한 숨김 쪽은 모든 사용자에게 공유되며, 원본 PDF와 사용자별 필기·책갈피는 삭제하지 않습니다. service role key는 환경변수나 시크릿 매니저에서만 사용하고 저장소에 커밋하지 마세요.

화면에 보이는 쪽 번호는 **숨긴 쪽을 뺀 순번**입니다. 공유 이미지의 파일 이름·공유 문구, 책갈피,
체크포인트, 공유 모달의 직접 입력까지 모두 같은 번호를 씁니다. 필기·책갈피·숨김 설정은 원본 PDF
쪽 번호로 저장하므로 숨김을 되살리면 기록이 그대로 제자리에 돌아옵니다.

### 바인더 파트와 실측 구조 메타데이터

목록에 보이는 항목은 "파트"입니다. 한 PDF를 여러 파트로 나눌 수 있어(새신자 PDF의 에다 기도문
71~94쪽, 축사 기도문 95~112쪽), `src/binderLibrary.ts`가 PDF 파일(`pdfId`)·필기 저장 키(`workId`)·
화면 파트(`id`)를 따로 둡니다. 필기와 책갈피는 PDF 단위로 저장하므로 파트를 나눠도 기존 기록이
그대로 열립니다.

체크포인트와 쪽별 기본 입력칸은 PDF에서 실측해 만듭니다.

```bash
npm run scan:binder-structure           # 실측 결과만 출력
node scripts/scan_binder_structure.mjs --write   # 데이터 파일 생성
npm run test:binder-structure           # 쪽 범위·순서·좌표 검증
```

- `scripts/binder_checkpoints.mjs` — 성경묵상은 3쪽 "성경묵상 순서"의 성경 단위, 디모데는
  "디모데 만들기 순서"의 대제목, 책공부는 책 단위입니다. 새신자 계열은 `binderCheckpoints`
  상수(원본 00-01호 쪽번호)를 그대로 씁니다.
- `scripts/binder_text_presets.mjs` — 성경묵상 필기 쪽의 묵상 3칸·배우자 기도 칸과, 디모데·책공부
  괘선 쪽을 덮는 큰 칸입니다. 값은 쪽 대비 비율이고 사용자가 실제로 입력하기 전에는 저장되지
  않습니다. `opaque` 칸은 타이핑할 때 인쇄된 괘선을 흰 바탕으로 가립니다(손글씨 모드에서는 줄이
  그대로 보입니다).

00-01호의 "순서" 쪽은 스캔 이미지라 텍스트 레이어가 없어 스크립트 안의 수동 보정 값을 씁니다.
PDF를 다시 만들면 두 데이터 파일을 재생성하고 테스트를 돌려야 합니다.
...
한국어 원문으로 fallback하며 번역문을 자동 생성하지 않습니다.

새 설교의 게시 상태 기본값은 **게시**입니다. 비공개(초안)로 저장한 설교는 `sermons`의 RLS가
`published or 관리자`라서 등록한 관리자의 관리자 모드에서만 보입니다. 등록 폼에서 비공개를 고르면
그 사실을 경고로 알립니다.
